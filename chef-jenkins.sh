...
CHEF_IMAGE=${CHEF_IMAGE:-90cb424a-ecfe-40e9-a149-918ef235ddb9}
        ...
        if [ ${NOCLEAN} -eq 0 ] || [ ${retval} -eq 0 ]; then
            if [ -e ${TMPDIR}/nodes ]; then
                for d in ${TMPDIR}/nodes/*; do
                    source ${d}
                    if [ ${DEPLOY} -ne 1 ]; then
                        print_banner "Node Data Dump for ${NODE_FRIENDLY_NAME}"
                        nova show "${NODE_FRIENDLY_NAME}"
...
function create_chef_environment() {
    ...
    knife environment from file ${environment_source} -c ${knife}
}
